...
intTree env (Node (Let v e1 e2) (DecomposeStep [t1, t2])) = 
	intTree ((v, intTree env t1) : env) t2

intTree env (Node (Ctr cname _) (DecomposeStep ts)) =
	Ctr cname $ map (intTree env) ts
	
intTree env (Node _ (TransientStep t)) = 
	intTree env t

intTree env (Node e (ContractStep cs)) = 
